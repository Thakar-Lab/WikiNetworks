...
def makeMatchedDistHist(featureDFs, pathwayID):
    ...
    plt.title(
        " ".join(
            ["Distribution of distance between endpoint and matched node: ", pathwayID]
        )
    )
...
def makeCombinedEndPointBarPlot(interactDFs=None):
    ...
    node_count = pd.DataFrame(
        concatInteractDF.groupby(by="PathwayID")["matched_Node1_cat"].value_counts(
            normalize=True
        )
    )
    node_count["matched_Node2_cat"] = concatInteractDF.groupby(by="PathwayID")[
        "matched_Node2_cat"
    ...
    plt.tight_layout()
    plt.savefig(
        "combined_endpoint_categories_1.png", bbox_inches="tight", transparent=True
    )
    plt.clf()
    ...
    temp = [
        "Explicit match"
        if list(concatInteractDF.node1_graphref)[i]
        == list(concatInteractDF.matched_Node1)[i]
        else "Inferred match"
        for i in range(len(concatInteractDF))
    ]
    # temp =
    concatInteractDF = concatInteractDF.assign(Node1_match=temp)
    concatInteractDF = concatInteractDF.assign(
        Node2_match=np.where(
            concatInteractDF.node2_graphref == concatInteractDF.matched_Node2,
            "Explicit match",
            "Inferred match",
        )
    )
    explicitMatch = pd.DataFrame(
        concatInteractDF.groupby(by="PathwayID")["Node1_match"].value_counts(
            normalize=True
        )
    )
    explicitMatch["Node2_match"] = concatInteractDF.groupby(by="PathwayID")[
        "Node2_match"
    ].value_counts(normalize=True)
    ...
    plt.tight_layout()
    plt.savefig(
        "combined_endpoint_categories_2.png", bbox_inches="tight", transparent=True
    )
    plt.clf()
...
def makeEndPointBarPlot(featureDFs, pathwayID):
    ...
    # Endpoint 1
    node_count1 = pd.DataFrame(
        featureDFs["interactDF"]["matched_Node1_cat"].value_counts()
    )
    node_count1 = node_count1.rename(columns={"matched_Node1_cat": "Endpoint 1"})

    # Endpoint 2
    node_count2 = pd.DataFrame(
        featureDFs["interactDF"]["matched_Node2_cat"].value_counts()
    )
    ...
    node_count = pd.melt(
        node_count.reset_index(),
        id_vars="index",
        value_vars=["Endpoint 1", "Endpoint 2"],
    )
...
def makeCombinedInteractionBarPlot(concatInteractDF):
    concatInteractDF = concatInteractDF.assign(
        OriginalEdge=[
            True if len(str(edge)) == 5 else False
            for edge in list(concatInteractDF.edgeID)
        ]
    )
    edge_count = pd.DataFrame(
        concatInteractDF.groupby(by="PathwayID")["OriginalEdge"].value_counts(
            normalize=True
        )
    )
    ...
    edge_count.OriginalEdge = [
        str("Explicit Edge") if temp == True else str("Inferred Edge")
        for temp in edge_count.OriginalEdge
    ]
    sns.catplot(
        x="value",
        y="OriginalEdge",
...
def evaluateProgramOutput():
    ...
    manualOuts = [
        "WP49_edgeList_edited.csv",
        "WP127_edgeList_edited.csv",
        "WP195_edgeList_edited.csv",
        "WP205_edgeList_edited.csv",
        "WP231_edgeList_edited.csv",
        "WP286_edgeList_edited.csv",
        "WP23_edgeList_edited.csv",
        "WP3935_edgelist_edited.csv",
        "WP4482_edgelist_edited.csv",
        "WP4868_edgelist_edited.csv",
        "WP4462_edgelist_edited.csv",
        "WP2038_edgelist_edited.csv",
        "WP453_edgelist_edited.csv",
        "WP545_edgelist_edited.csv",
        "WP306_edgelist_edited.csv",
    ]
    fileNames = pd.DataFrame(
        list(zip(programOuts, manualOuts)), columns=["programOuts", "manualOuts"]
    )
    ...
    netDiffResult.columns = [
        "number_of_manualNodes",
        "number_of_programNodes",
        "manualNodes",
        "programNodes",
        "extra_manualNodes",
        "extra_programNodes",
        "number_of_extra_manualEdges",
        "number_of_extra_programEdges",
        "extraEdgesInManual",
        "extraEdgesInProgram",
        "propTrueNodes",
        "propTrueEdges",
...
def compareCytoscapeWithProgramOutput():
    ...
    manualOuts = [
        ...
        "WP286_cytoscape.csv",
        "WP23_cytoscape.csv",
        "WP3935_cytoscape.csv",
        "WP4868_cytoscape.csv",
        "WP4482_cytoscape.csv",
        "WP4462_cytoscape.csv",
        "WP2038_cytoscape.csv",
        "WP453_cytoscape.csv",
        "WP545_edgelist_edited.csv",
        "WP306_edgelist_edited.csv",
    ]
    fileNames = pd.DataFrame(
        list(zip(programOuts, manualOuts)), columns=["programOuts", "cytoscapeOuts"]
    )
    fileList = dict(zip(programOuts, manualOuts))
    ...
    netDiffResult.index = programOuts
    netDiffResult.columns = [
        "number_of_cytoscapeNodes",
        "number_of_programNodes",
        "cytoscapeNodes",
        "programNodes",
        "extra_cytoscapeNodes",
        "extra_programNodes",
        "number_of_extra_cytoscapeEdges",
        "number_of_extra_programEdges",
        "extraEdgesIncytoscape",
        "extraEdgesInProgram",
        "propTrueNodes",
        "propTrueEdges",
        "truePositiveEdges",
...
def createCombinedEdgelists():
    ...
    for cel in cytoscapeEdgelists:
        ...
        consensus_edgelist = consensus_edgelist[
            ~consensus_edgelist.Source.str.contains("group", na=False)
        ]
        consensus_edgelist = consensus_edgelist[
            ~consensus_edgelist.Target.str.contains("group", na=False)
        ]
        consensus_edgelist = consensus_edgelist.drop_duplicates(
            subset=["Source", "Target", "signal"]
        )
...
def evaluateCytoscapeOutput():
    programOuts = [
        "WP49_cytoscape.graphml",
        "WP127_cytoscape.graphml",
        "WP195_cytoscape.graphml",
        "WP205_cytoscape.graphml",
        "WP231_cytoscape.graphml",
        "WP286_cytoscape.graphml",
        "WP23_cytoscape.graphml",
        "WP3935_cytoscape.graphml",
        "WP4868_cytoscape.graphml",
        "WP4482_cytoscape.graphml",
        "WP4462_cytoscape.graphml",
        "WP2038_cytoscape.graphml",
        "WP453_cytoscape.graphml",
        "WP545_cytoscape.graphml",
...
def evaluateCytoscapeOutput_fairplay():
    ...
    for i in range(len(programOuts)):
        ...
        nx.write_edgelist(
            graph, programOuts[i], delimiter="\t", data=["targetArrows"]
        )  #
        # print(programOuts[i])
...
def makeEvaluationPlots(netDiffResult, softwareName="wikinetworks"):
    ...
    netDiffResult["Pathway"] = [
        re.sub("_edgeList.csv", "", temp) for temp in netDiffResult.index
    ]
    ax = sns.barplot(
        x="propTrueEdges", y="Pathway", data=netDiffResult, color="#6495ED"
    )
    plt.xlim(0, 1)
    ...
    ax = sns.barplot(
        x="propTrueNodes", y="Pathway", data=netDiffResult, color="#6495ED"
    )
    ...
    ax = sns.barplot(
        y="number_of_extra_programEdges",
        x="Pathway",
        data=netDiffResult,
        color="#6495ED",
    )
    ...
    ax = sns.barplot(
        y="extra_programNodes", x="Pathway", data=netDiffResult, color="#6495ED"
    )
    ...
    ax = sns.barplot(
        y="extra_manualNodes", x="Pathway", data=netDiffResult, color="#6495ED"
    )
    ax.figure.savefig("extra_manualNodes" + "_" + str(softwareName) + ".png")
    plt.clf()
    # Number of true positive edges
    ax = sns.barplot(
        y="truePositiveEdges", x="Pathway", data=netDiffResult, color="#6495ED"
    )
    ...
    for i in range(netDiffResult.shape[0]):
        plt.text(
            x=netDiffResult.recall[i],
            y=netDiffResult.precision[i],
            s=netDiffResult.Pathway[i],
...
def makeComparisonPlots():
    ...
    wikinetworks = pd.read_csv(
        "netDiffResult_wikinetworks.tsv", sep="\t", index_col=0, escapechar="\\"
    )
    ...
    # print(combinedDF)
    combinedDF["Pathway"] = [
        re.sub("_edgeList.csv", "", temp) for temp in combinedDF.index
    ]
    combinedDF["Pathway"] = [
        re.sub("_cytoscape.csv", "", temp) for temp in combinedDF["Pathway"]
    ]
    # combinedDF["Pathway"] = [re.sub("fairplay_", "", temp) for temp in combinedDF["Pathway"]]
    sns.set_context(
        "paper"
    )  # , rc={"font.size":10,"axes.titlesize":12,"axes.labelsize":12})
    # plt.figure(figsize=(8,5))
    ax = sns.barplot(
        x="precision", y="Pathway", hue="Method", data=combinedDF, palette="Blues"
    )
    plt.xlim(0, 1)
    ...
    ax = sns.barplot(
        x="precision", y="Method", data=combinedDF, palette="Blues", ci="sd"
    )
    plt.xlim(0, 1)
    plt.xlabel("Precision")
    ...
    ax = sns.barplot(
        x="f1Score", y="Pathway", hue="Method", data=combinedDF, palette="Purples"
    )
    plt.xlim(0, 1)
    plt.xlabel("f1Score")
    ...
    sns.set_context("paper")
    ax = sns.barplot(
        x="recall", y="Pathway", hue="Method", data=combinedDF, palette="OrRd"
    )
    plt.xlim(0, 1)
    plt.xlabel("Recall")
    ...
    fig, ax = plt.subplots()
    sns.scatterplot(
        x="recall", y="precision", data=combinedDF, hue="Method", palette="Set2", ax=ax
    )
    ...
    sns.set_context("paper")
    plt.figure(figsize=(10, 5))
    cDF2 = combinedDF.melt(
        id_vars=["Pathway", "Method"], value_vars=["precision", "recall", "f1Score"]
    )
